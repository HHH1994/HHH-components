<template>
  <div class="flex">
    ...
    <el-cascader
      ...
      clearable
      collapse-tags
      :popper-class="'ys-cascader' + packId + '-' + componentData.menuId"
  ...
  methods: {
    ...
    cascaderLabelClick() {
      this.$nextTick(() => {
        document
          .querySelectorAll(
            `.ys-cascader${
              this.packId + "-" + this.componentData.menuId
    ...
    setTitle() {
      ...
      this.$nextTick(() => {
        let box = document.querySelectorAll(
          `.ys-cascader${this.packId + "-" + this.componentData.menuId} ul`
        );
        box.forEach((i, index) => {
    ...
    emitData() {
      ...
      let data = {
        ...
        tagName: this.showNames.join(",") || "",
        labelId: obj.id ? [obj] : [],
        realValues: this.cascarValue,
        sort: this.componentData.sort,
        showSingle: this.componentConf.showSingle,
      };
      console.log(data, "data99999999999");
      this.$emit("change", data, this.componentData);
    },
  },
};
</script>
